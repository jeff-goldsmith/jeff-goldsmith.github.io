...
This page hosts a presentation / quick intro to `tidyfun` -- enjoy!
...
The code in this page is drawn from [`tidyfun`](https://fabian-s.github.io/tidyfun/) vignettes, and is intended as a quick introduction; for more details, please read the complete documentation!
...
- Includes additional attributes that define *function-like* behavior:
    - how to **evaluate** the given "functions" for new arguments
    - their **domain** 
    - the **resolution** of the argument values

First I'll pull a `tf` vector from the `tidyfun::dti_df` dataset, and then I'll extract a simple 5-element vector of functions interpolated to be on a regular grid. The resulting vector contain fractional anisotropy tract profiles for the corpus callosum (`cca`). When printed, `tf` vectors show the first few `arg` and `value` pairs for each subject.  

```{r load-dti}
data("dti_df")

cca_five = dti_df$cca[1:5]
cca_five = tfd(cca_five, arg = seq(0, 1, l = 93), interpolate = TRUE)
...
It's possible to convert between types.
...
A quick plot illustrates the difference visually; lots of things are different in the background!

```{r}
plot(cca_five)
lines(cca_five_b, col = "red")
```
...
You can smooth data using `tf_smooth`; this differs from converting to subclass `tfb`:
...
plot(cca_five)
lines(cca_five_smooth, col = "blue")
```
